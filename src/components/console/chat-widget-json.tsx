...
export default function ChatWidgetJson({ key, item, active = false, label = 'JSON' }: { key: string; item: { _out: string }; active?: boolean }) {
  ...
  return (
        ...
        <span key={key} className={`flex flex-col mb-4 w-full max-w-[75%] mx-auto ${!active ? 'opacity-50' : ''}`}>
            <div className="text-xs text-gray-400 bg-neutral-800/70 font-mono px-2 py-2 rounded-t flex flex-row items-center gap-1">{label} <Braces className="h-4 w-4" /></div>
